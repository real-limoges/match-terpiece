...
def scrape_tag_public(TAG, collection, min_date, max_date):
    ...
    for page in xrange(1, pages + 1):
        ...
        if len([photo for photo in response.find('photos')]) == 0:
            return

        added = 0
        for photo in response.find('photos'):
            link = create_link(photo)
